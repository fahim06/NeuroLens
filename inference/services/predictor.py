...
class MultiDomainPredictorService:
    ...
    def _mock_human_animal(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "all_classes": [
                {"label": "Human", "confidence": round(confidence if is_human else 1-confidence, 4)},
                {"label": "Animal", "confidence": round(1-confidence if is_human else confidence, 4)}
            ]
        }
    
    def _mock_animal_category(self) -> Dict[str, Any]:
        """Mock animal category detection."""
        categories = ["Dog", "Cat", "Bird", "Fish", "Reptile", "Mammal", "Insect", "Primate"]
    ...
    def _mock_brain_tumor(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            "is_tumor": is_tumor,
            "all_classes": [
                {"label": "No Tumor", "confidence": round(1-confidence if is_tumor else confidence, 4)},
                {"label": "Tumor", "confidence": round(confidence if is_tumor else 1-confidence, 4)}
            ]
        }
        
        # Add medical note
        if is_tumor:
            result["medical_note"] = "Potential tumor detected. Consult physician for confirmation."
        else:
            result["medical_note"] = "No tumor indicators detected. Regular checkups recommended."
    ...
    def _mock_citrus(self) -> Dict[str, Any]:
        ...
        citrus_types = [
            "Limon Criollo", "Limon Mandarino", "Mandarina Pieldesapo",
            "Mandarina Israeli", "Naranja Valencia", "Tangelo", "Toronja", "Lima"
        ]
        ...
        return {
            ...
            "top_predictions": [
                ...
                {"label": other_types[0], "confidence": round(random.uniform(0.01, 0.08), 4)},
                {"label": other_types[1], "confidence": round(random.uniform(0.01, 0.05), 4)}
            ]
        }
    ...
    def _mock_auto_analyze(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "prediction": prediction,
            "biological_classification": prediction.get("biological_classification") or prediction.get("hierarchy"),
            "timestamp": datetime.utcnow().isoformat(),
            "metadata": {
                "is_mock": True,
