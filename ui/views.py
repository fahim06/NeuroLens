...
def dashboard_view(request):
    ...
    accuracy = round((successful_requests / total_requests * 100), 1) if total_requests > 0 else 0
    ...
    recent_activity = InferenceRequest.objects.filter(
...
def reports_view(request):
    ...
    # Domain distribution
    domain_data = InferenceRequest.objects.filter(status='success').values('result').annotate(
        count=Count('id')
    )
    ...
    recent_requests = InferenceRequest.objects.select_related('requested_by').order_by('-created_at')[:10]

    return render(request, 'ui/reports.html', {
        'user': request.user,
        'analytics': {
    ...
    if request.method == 'POST':
        ...
        if profile_picture:
            ...
            if profile_picture.content_type not in allowed_types:
                errors.append('Profile picture must be a JPEG, PNG, GIF, or WebP image.')
    ...
    if request.method == 'POST':
        ...
        if current_password and not new_password and not confirm_password:
            if user.check_password(current_password):
                ...
                return JsonResponse({'valid': False, 'error': 'Current password is incorrect.'})
