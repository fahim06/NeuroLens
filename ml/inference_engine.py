...
try:
    ...
    logging.getLogger(__name__).warning(
        "NumPy not available - ML functionality disabled"
    )
...
class MultiDomainInferenceEngine:
    ...
    def predict(
        ...
        image = self._preprocess_image(
            image_data, config.input_size, config.preprocessing
        )
    ...
    def _preprocess_image(
        ...
        if not NUMPY_AVAILABLE:
            raise ImportError(
                "NumPy is required for image preprocessing but is not available"
            )
